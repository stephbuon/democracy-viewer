...
def get_text(engine: Engine, table_name: str) -> DataFrame:
    start = time()
...
def get_columns(table_name: str, columns: list[str]):
    df = download("datasets", table_name)
    
    return df[columns]

# Select records by group and word lists
def basic_selection(table_name: str, column: str | None, values: list[str], word_list: list[str]) -> DataFrame:
...
def group_count_by_words(table_name: str, word_list: list[str], column: str | None) -> dict[str, int]:
    # Download raw and tokenized data
    df_raw = download("datasets", table_name)
    df_split = download("tokens", table_name)
...
def group_count(table_name: str, column: str) -> int:
    # Download raw data
    df_raw = download("datasets", table_name)
...
def record_count(table_name: str) -> int:
    df = download("datasets", table_name)
        
    return len(df)

# Get the total word count in a corpus
def total_word_count(table_name: str) -> int:
...
def word_counts(table_name: str, word_list: list[str]) -> dict[str, int]:
    # Download tokenized data
    df_split = download("tokens", table_name)
...
def group_counts(table_name: str, column: str, values: list[str]) -> dict[str, int]:
    # Download raw and tokenized data
    df_raw = download("datasets", table_name)
    df_split = download("tokens", table_name)
