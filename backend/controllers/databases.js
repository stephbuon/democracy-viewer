...
const newConnection = async(databases, name, owner, is_public, host, port, db, username, password, client) => {
...
const loadConnection = async(databases, id) => {
    // Get connection credentials by id
    const creds = databases.getCredentials(id);
    // Decrypt credentials
